...
def logsumexp_by_id_pt(semantic_ids, log_likelihoods, agg='sum_normalized'):
    """Sum probabilities with the same semantic id.

    Log-Sum-Exp because input and output probabilities in log space.
    semantic_ids: List of semantic ids, e.g. [0, 1, 2, 1].
    log_likelihoods: List of log likelihoods, e.g. tensor([-1.3813, -0.5855, -1.8969, -1.1646], device='cuda:0'.
    """
    semantic_ids = torch.tensor(semantic_ids, device=log_likelihoods.device)
    unique_ids = torch.unique(semantic_ids)
    log_likelihood_per_semantic_id = []
    
    for uid in unique_ids:
        # Find positions in `semantic_ids` which belong to the active `uid`.
        mask = (semantic_ids == uid)
        # Gather log likelihoods at these indices.
        id_log_likelihoods = log_likelihoods[mask]
...
def predictive_entropy_rao_pt(log_probs):
